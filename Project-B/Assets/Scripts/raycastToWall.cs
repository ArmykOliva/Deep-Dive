...
public class RaycastGizmoDrawer : MonoBehaviour
{
  ...
  {
    RaycastHit hit;
    float maxDistance = 100f; // Set the max distance for the raycast
  ...
  private void OnDrawGizmos()
  {
    ...
    float maxDistance = 100f; // Ensure this is the same as in Update

    // Draw the ray line
    if (hasHit)
    {
      Gizmos.DrawLine(tip.position,hitPoint); // Line to hit point
      Gizmos.DrawSphere(hitPoint, 0.5f); // Sphere at hit point
    }
    else
    {
